...
export default function ScoreboardView({ matchId, onClose }: Props) {
  ...
  const incrementScore = (team: 1 | 2) => {
    setT1Score(prev => team === 1 ? prev + 1 : prev);
    setT2Score(prev => team === 2 ? prev + 1 : prev);

    // Update Firestore in real-time
    updateMatchScore(match.id, [t1Score, t1Sets], [t2Score, t2Sets], false);
  };
  ...
  const winSet = (team: 1 | 2) => {
    if (team === 1) {
      setT1Sets(prev => prev + 1);
      setT1Score(0); // Reset current set score
    } else {
      setT2Sets(prev => prev + 1);
      setT2Score(0); // Reset current set score
    }
